...
import { getDeckProgress, totalMastered, getStreak } from '../lib/progress.js';

export default function HomeScreen({ onPickTopic }) {
  const [filter, setFilter] = useState('all');
  // simple version key so we re-read localStorage when returning from study
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === 'inprogress') return enriched.filter((t) => t.known > 0 && t.known < t.count);
    if (filter === 'new') return enriched.filter((t) => t.known === 0);
    if (filter === 'mastered') return enriched.filter((t) => t.count > 0 && t.known / t.count >= 0.8);
    return enriched;
  }, [enriched, filter]);
  ...
  return (
    <div className="page">
      <section className="hero">
        ...
        <p>
          Bite-sized flashcards across every red-dot topic on the CEN&nbsp;4020 final review. Flip,
          swipe, or tap your way to a perfect score — five minutes at a time.
        </p>
...
function GlyphBg({ shape, color }) {
  ...
  switch (shape) {
    case 'wave':
      path = (
        <path d="M5 50 Q 25 20, 50 50 T 95 50" stroke={stroke} strokeWidth={sw} fill="none" strokeLinecap="round" />
      );
      break;
    case 'sprint':
      path = (
    ...
    case 'scale':
      ...
      break;
    default:
      path = <circle cx="50" cy="50" r="30" stroke={stroke} strokeWidth={sw} fill="none" />;
  }
  return (
